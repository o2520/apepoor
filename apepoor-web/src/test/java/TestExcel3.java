...
public class TestExcel3 {
    ...
    public static void main(String[] args) {
        ...
        reader0.setSheet(0);
        List<ProductDTO> list = reader0.readAll(ProductDTO.class);
        Map<String, ProductDTO> productMap = list.stream().collect(Collectors.toMap(ProductDTO::getProductNo, productDTO -> productDTO));
        ...
        resultList.parallelStream().forEach(productDTO -> {
            ProductDTO dto = productMap.get(productDTO.getProductNo());
            if(null != dto){
                productDTO.setName(dto.getName());
                productDTO.setKey(dto.getKey());
            }
        });
